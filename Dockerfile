...
COPY package*.json ./
RUN npm install -g npm@latest && npm ci --frozen-lockfile
...
RUN apk add --no-cache libc6-compat openssl postgresql-client iputils bind-tools traceroute fping && \
...
COPY --from=builder --chown=nextjs:nodejs /app/node_modules/@ldapjs  ./node_modules/@ldapjs
COPY --from=builder --chown=nextjs:nodejs /app/node_modules/busboy   ./node_modules/busboy
