      ...
      if (result.error) toast.error(result.error);
        ...
        setComments((prev) => [
          ...prev,
          {
            ...
            author: {
              name:
                workspaceMembers.find((m) => m.id === currentUserId)?.name ?? "You",
            },
            createdAt: new Date(),
          },
        ]);
        ...
        onTaskUpdate({
          ...task,
          _count: { ...task._count, comments: task._count.comments + 1 },
        });
      }
    });
  };
  ...
  const handleDeleteComment = (commentId: string) => {
    startTransition(async () => {
      ...
      if (result.error) toast.error(result.error);
        ...
        onTaskUpdate({
          ...task,
          _count: { ...task._count, comments: Math.max(0, task._count.comments - 1) },
        });
      }
    });
  };
  ...
  const handleAddCheckItem = () => {
    ...
    startTransition(async () => {
      ...
      if (result.error) toast.error(result.error);
        ...
        onTaskUpdate({
          ...task,
          _count: { ...task._count, checklist: task._count.checklist + 1 },
        });
      }
    });
  };
  ...
  const handleDeleteCheckItem = (itemId: string) => {
    startTransition(async () => {
      ...
      if (result.error) toast.error(result.error);
        ...
        onTaskUpdate({
          ...task,
          _count: { ...task._count, checklist: Math.max(0, task._count.checklist - 1) },
        });
      }
    });
  };
    ...
    startTransition(async () => {
      if (hasLabel) {
        ...
        if (result.error) toast.error(result.error);
          ...
          onTaskUpdate({
            ...task,
            labels: task.labels.filter((l) => l.label.id !== labelId),
          });
      } else {
          ...
          <div className="flex-1 pr-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              ...
              <span className="font-medium text-foreground">
                {allColumns.find((c) => c.id === task.columnId)?.name}
              </span>
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-6">
            <div>
              ...
              {editDesc ? (
                ...
                <div
                  className={cn(
                    "text-sm text-muted-foreground rounded-md p-2 min-h-[40px]",
                    canEdit && "cursor-pointer hover:bg-accent hover:text-foreground transition-colors"
                  )}
                  onClick={() => canEdit && setEditDesc(true)}
                >
                  {description || (
                    <span className="italic">{canEdit ? "Click to add description..." : "No description"}</span>
                  )}
                </div>
              )}
            </div>
                  ...
                  <div className="space-y-1.5">
                    {checklist.map((item) => (
                      <div key={item.id} className="flex items-center gap-2 group/check">
                        <Checkbox
                          ...
                          onCheckedChange={() => canEdit && handleToggleCheck(item.id)}
                        ...
                        {canEdit && (
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-6 w-6 opacity-0 group-hover/check:opacity-100"
                    ...
                    {showAddCheck && canEdit && (
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        />
                        <Button size="sm" className="h-7" onClick={handleAddCheckItem}>
                          Add
                        </Button>
                        <Button
                          size="sm"
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {comment.authorId === currentUserId && (
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-5 w-5 opacity-0 group-hover/comment:opacity-100 ml-auto"
                  ...
                  {canEdit && (
                    <div className="flex gap-3">
                      <Avatar className="h-7 w-7 shrink-0 mt-0.5">
                        <AvatarFallback className="text-[10px] bg-primary/20 text-primary">
                          {getInitials(
                            workspaceMembers.find((m) => m.id === currentUserId)?.name ?? "?"
                          )}
                        </AvatarFallback>
                      </Avatar>
                      <div className="flex-1">
                        ...
                        {newComment.trim() && (
                          <div className="flex gap-2 mt-2">
                            <Button size="sm" className="h-7" onClick={handleAddComment} disabled={isPending}>
                              Comment
                            </Button>
                            <Button
                              size="sm"
          ...
          <div className="w-56 shrink-0 border-l border-border p-4 space-y-5 overflow-y-auto">
            <div>
              ...
              {canEdit ? (
                  ...
                  <SelectContent>
                    {["LOW", "MEDIUM", "HIGH", "URGENT"].map((p) => (
                      <SelectItem key={p} value={p}>
                        <span className={cn("font-medium", PRIORITY_COLORS[p].split(" ")[0])}>
                          {PRIORITY_LABELS[p]}
                        </span>
                      </SelectItem>
                    ))}
                ...
                <span className={cn("text-sm font-medium", PRIORITY_COLORS[task.priority].split(" ")[0])}>
